...
Date    : 2023/7/19
Usage   : Data structure of PRB SPEC
'''
...
class PrbSpec:
    ...
    pin_head_label = ['Select', 'PadNumber', 'PinNumber', 'Comment']
    ...
    def generateXMLTree(self):
        root = ET.Element("ProbeSpec")
        root.set('Name', '{}'.format(self.Name))

        # for sub node
        pad_pos = ET.SubElement(root, "PadPin")
        des = ET.SubElement(root, "Description")

        # for die sub node
        space_list_body = []
        for i in range(len(self.pin_head_label)):
            max_con = max(len(row[i]) for row in self.body_con)
            space_list_body.append(max(len(self.pin_head_label[i]), max_con))
        device_pos_head = ET.SubElement(pad_pos, "Head")
        head_content = []
        for i in range(len(self.pin_head_label)):
            str = '[{}] '.format(self.pin_head_label[i].ljust(space_list_body[i]))
        ...
        for it in self.body_con:
            ...
            for i in range(len(it)):
                ...
                content.append(str)
            item = ET.SubElement(pad_pos, "Item")
...
if __name__ == '__main__':
    ...
    waf = PrbSpec(file_attribute=file_attribute, body_content=body_content)
    waf.saveAsXml('d.txt')
