...
Date    : 2023/7/19
Usage   : Data structure of DIE SPEC
'''
...
class DieSpec:
    ...
    module_head_label = ['Select', 'Module', 'X', 'Y', 'Comment']
    ...
    def generateXMLTree(self):
        root = ET.Element("DieSpec")
        root.set('Name', '{}'.format(self.Name))

        # for sub node
        module_pos = ET.SubElement(root, "Modules")
        des = ET.SubElement(root, "Description")

        # for die sub node
        space_list_body = []
        for i in range(len(self.module_head_label)):
            max_con = max(len(row[i]) for row in self.body_con)
            space_list_body.append(max(len(self.module_head_label[i]), max_con))
        module_pos_head = ET.SubElement(module_pos, "Head")
        head_content = []
        for i in range(len(self.module_head_label)):
            str = '[{}] '.format(self.module_head_label[i].ljust(space_list_body[i]))
            head_content.append(str)
        module_pos_head.text = ''.join(head_content)
        for it in self.body_con:
            content = []
            for i in range(len(it)):
                str = ' {}  '.format(it[i].ljust(space_list_body[i]))
                content.append(str)
            item = die_pos_head = ET.SubElement(module_pos, "Item")
...
if __name__ == '__main__':
    ...
    waf = DieSpec(file_attribute=file_attribute, body_content=body_content)
    waf.saveAsXml('b.txt')
